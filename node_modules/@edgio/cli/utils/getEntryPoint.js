...
module.exports = async function getEntryPoint(name) {
  let connector = getEdgioConfig().connector || '@edgio/core';
  let entryPoint;
  ...
  const mod = entryPoint.endsWith('.mjs') ? Promise.resolve().then(() => _interopRequireWildcard(require(`${slash(entryPoint)}`))) : require(slash(entryPoint));
