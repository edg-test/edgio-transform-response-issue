...
async function initializeApp(args) {
  ...
  let customConnectorConfig = undefined;

  if (args.connector === undefined) {
    customConnectorConfig = await askForCustomConnector(logger);

    if (customConnectorConfig) {
      args.connector = {
        key: 'edgio-custom-connector',
        name: 'Edgio Custom Connector',
        builder: '@edgio/custom-connector',
        devDependencies: ['@edgio/custom-connector'],
        framework: 'custom',
  ...
  !args.skipConfigUpdate && (await updateConfig(args)); // If custom-connector is set, update the config with the custom connector settings
  ...
  if (customConnectorConfig) {
    setConfig(customConnectorConfig);
  }
...
const askForCustomConnector = async logger => {
  logger.warn('WARNING: No framework detected. You can still use Edgio, we just need more information about your project');
  ...
  const buildFolder = await prompts([{
    type: 'text',
    name: 'buildFolder',
    message: 'What is the build directory of your app?',
    initial: 'dist'
  }]);
  const entryFile = await prompts([{
    type: 'text',
    name: 'entryFile',
    message: 'What is the entry file of your app',
    initial: 'index.js'
  }]);
  const envPort = await prompts([{
    type: 'text',
    name: 'envPort',
    message: 'What is the envirovment variable name for port (if it is available)?',
    initial: 'PORT'
  }]);
  const buildCommand = await prompts([{
    type: 'text',
    name: 'buildCommand',
    message: 'Please specify build command if exists'
  }]);
  const devCommand = await prompts([{
    type: 'text',
    name: 'devCommand',
    message: 'Please specify dev command if exists'
  }]);
  const devReadyMessageOrTimeout = await prompts([{
    type: 'text',
    name: 'devReadyMessageOrTimeout',
    message: 'Please specify dev ready message or timeout value in seconds to wait'
  ...
  const customConnectorConfig = {
    customConnector: {
      ...
      devReadyMessageOrTimeout: devReadyMessageOrTimeout.devReadyMessageOrTimeout
    }
  };
  return customConnectorConfig;
};