...
class CustomCacheKey {
    ...
    applyCaching(features) {
        ...
        const shouldOverwriteCache = this.headersFlag.length > 0 ||
        ...
        if (this.excludeAllQueryParametersFlag) {
            caching.cache_key_query_string = {
                exclude_all: this.excludeAllQueryParametersFlag,
            };
        }
        if (this.excludeQueryParametersFlag.length > 0) {
            caching.cache_key_query_string = {
                ...caching.cache_key_query_string,
                include_all_except: this.excludeQueryParametersFlag,
            };
        }
        if (this.excludeAllQueryParametersExceptFlag.length > 0) {
            caching.cache_key_query_string = {
                ...caching.cache_key_query_string,
                include: this.excludeAllQueryParametersExceptFlag,
            };
        }
        if (this.headersFlag.length > 0) {
            caching.cache_key_rewrite.destination += this.headersFlag
                .sort()
                .map(name => `/header_${name}:%{http_${name}}`)
                .join('');
        }
        if (this.cookiesFlag.length > 0) {
            caching.cache_key_rewrite.destination += this.cookiesFlag
                .sort()
                .map(name => `/cookie_${name}:%{cookie_${name}}`)
                .join('');
        }
        if (this.deviceFlag) {
            caching.cache_key_rewrite.destination += `/device:smartphone_%{wurfl_vcap_is_smartphone}_tablet_%{wurfl_cap_is_tablet}_desktop_%{wurfl_vcap_is_full_desktop}`;
        }
        if (this.isBotFlag) {
            caching.cache_key_rewrite.destination += `/bot:%{wurfl_vcap_is_robot}`;
        }
        if (this.vendorFlag) {
            caching.cache_key_rewrite.destination += `/vendor:ios_%{wurfl_vcap_is_ios}_adndroid_%{wurfl_vcap_is_android}`;
        }
        if (this.browserFlag) {
            caching.cache_key_rewrite.destination += `/browser:%{wurfl_cap_mobile_browser}`;
        }
